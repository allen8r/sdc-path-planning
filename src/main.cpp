...
int main() {
  ...
  while (getline(in_map_, line)) {
	...
	double ln_width = 4;
	double ln_half = ln_width / 2;
	int lane = 1; // middle lane

  h.onMessage([&ref_vel, &ln_width, &ln_half, &lane, &map_waypoints_x,&map_waypoints_y,&map_waypoints_s,&map_waypoints_dx,&map_waypoints_dy](uWS::WebSocket<uWS::SERVER> ws, char *data, size_t length,
                     uWS::OpCode opCode) {
          ...
          // j[1] is the data JSON object
        	...
        	// Main car's localization Data
          	...
          	// TODO: define a path made up of (x,y) points that the car will visit sequentially every .02 seconds
						...
						// 1. Just get the car going in a straight line
          	...
          	vector<double> next_y_vals;
						...
						for(int i = 0; i < 50; i++)
						{
							...
							double next_d = lane * ln_width + ln_half;
						...
						for (int i= 0; i < sensor_fusion.size(); i++)
						{
							...
							if (d < (ln_half + ln_width*lane + ln_half) && d > (ln_half + ln_width*lane - ln_half))
							{
								...
								if ((check_car_s > car_s) && ((check_car_s - car_s) < 30)) // closer than 30m
								{
									// React because ego car is now too close to other car.
						...
						{
							ref_vel += 0.224; // accelerate by increments up to max velocity
						}

						// 3. Smooth out the trajectory curve
						vector<double> ptsx;
						vector<double> ptsy;
						...
						double s_inc = 30;
						vector<double> next_wp0 = getXY(car_s+s_inc*1, (ln_half+ln_width*lane), map_waypoints_s, map_waypoints_x, map_waypoints_y);
						vector<double> next_wp1 = getXY(car_s+s_inc*2, (ln_half+ln_width*lane), map_waypoints_s, map_waypoints_x, map_waypoints_y);
						vector<double> next_wp2 = getXY(car_s+s_inc*3, (ln_half+ln_width*lane), map_waypoints_s, map_waypoints_x, map_waypoints_y);
